...
export function SensorFocusPanel() {
  ...
  const [centerMode, setCenterMode] = useState<"analysis" | "statistic">(
    "analysis",
  );
  ...
  const nowMs = useMemo(() => {
    if (sensorHistory.length > 0) {
      const validTimes = sensorHistory
        .map((h) => new Date(h.timestamp).getTime())
        .filter((t) => !isNaN(t));

      return validTimes.length > 0 ? Math.max(...validTimes) : currentTime;
    }
    return currentTime;
  ...
  const tableData = useMemo(() => {
    ...
    return sensorHistory
      .filter((h) => {
        ...
        return !isNaN(t) && t >= sixHoursAgo && t <= nowMs;
      })
      .sort(
        (a, b) =>
  ...
  const toggleAnalysis = () => {
    if (isCenterOpen && centerMode === "analysis") {
      setIsCenterOpen(false);
    } else {
      setCenterMode("analysis");
      setIsCenterOpen(true);
    }
  };

  const toggleStatistic = () => {
    if (isCenterOpen && centerMode === "statistic") {
      setIsCenterOpen(false);
    } else {
      setCenterMode("statistic");
      setIsCenterOpen(true);
    }
  };
    ...
    <>
      ...
      >
        <div className="flex justify-between items-center w-full bg-white/95 backdrop-blur-md rounded-xl shadow-sm border border-gray-200/80 p-2 shrink-0 sticky top-0 z-50 mb-2">
          ...
          {/* Hide navigation and close buttons when collapsed */}
          {!isLeftCollapsed && (
      ...
      >
        <div className="flex justify-between items-center w-full bg-white/95 backdrop-blur-md rounded-xl shadow-sm border border-gray-200/80 p-2 shrink-0 sticky top-0 z-50 mb-2 ">
          <div className="flex items-center gap-2 px-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-800 p-1.5">
              Live Telemetry
            </span>
          </div>
          {forecast && (
            <Badge
              variant="outline"
              className="border-amber-500 text-amber-600 bg-amber-50 gap-1 px-2 pointer-events-none p-1.5"
            >
      ...
      {/* CENTER PANELS WITH INTEGRATED TOGGLE BUTTON GROUP (DOCKED TO RIGHT) */}
      <div className="absolute top-20 right-[472px] z-40 flex flex-col items-end pointer-events-none">
        {/* BUTTON GROUP TOGGLE */}
        <div className="flex items-center gap-1 pointer-events-auto bg-white/95 backdrop-blur-md rounded-xl shadow-md border border-gray-200/80 p-1 relative z-50">
          <button
            onClick={toggleStatistic}
            className={`p-1.5 rounded-lg flex items-center justify-center transition-all duration-300 ${
              isCenterOpen && centerMode === "statistic"
                ? "bg-[#1e3a8a] text-white shadow-sm"
                : "text-[#1e3a8a] hover:bg-blue-50"
            }`}
            title="Telemetry Log (Last 6 Hours)"
          >
            {isCenterOpen && centerMode === "statistic" ? (
              <X size={18} />
            ) : (
          ...
          <button
            ...
            className={`p-1.5 rounded-lg flex items-center justify-center transition-all duration-300 ${
              isCenterOpen && centerMode === "analysis"
                ? "bg-[#1e3a8a] text-white shadow-sm"
                : "text-[#1e3a8a] hover:bg-blue-50"
            }`}
        ...
        >
          <div className="flex-1 w-full overflow-y-auto custom-scrollbar drop-shadow-2xl">
            {/* ANALYSIS MODE */}
            <div
              style={{ display: centerMode === "analysis" ? "block" : "none" }}
            >
              <HardwareReceiptCard
                ...
                >
                  ...
                  {(isTarmacTemp || isOutdoorWind) && (
                    <div className="flex flex-col gap-1">
                      <div className="flex justify-between items-center ml-8 mb-2 pr-4">
                        <span className="text-[9px] text-gray-400 uppercase font-bold tracking-widest">
                          Surface Thermal Dissipation: Tarmac Temp vs Outdoor
                          Wind
                        </span>
                        <SyncBadge />
                      </div>
            ...
            {/* STATISTIC MODE */}
            <div
              style={{ display: centerMode === "statistic" ? "block" : "none" }}
            >
              <HardwareReceiptCard
                title="Telemetry Log"
                vendorId={vendorId}
                icon={Table}
                footerText="LAST 6 HOURS"
                theme={theme}
              >
                <div className="flex flex-col w-full mt-2">
                  <div className="w-full bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex flex-col max-h-[650px]">
                          ...
                          {tableData.length > 0 ? (
                            tableData.map((row, idx) => (
                              ...
                              >
                                ...
                                <td className="px-3 py-2.5 text-right font-mono font-bold text-[#1e3a8a]">
                                  {row.value.toFixed(5)}
                            ...
                            <tr>
                              <td
                                colSpan={3}
                                className="px-3 py-8 text-center text-gray-400 font-bold uppercase tracking-widest"
                              >
                                No data logged in the last 6 hours
                              </td>
                            </tr>
                          )}
                        </tbody>

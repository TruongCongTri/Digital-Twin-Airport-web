...
export function ArcGISMap() {
  ...
  const syncTooltips = useCallback((view: SceneView) => {
    ...
    if (store.mapFilters["planes"] !== false) {
      store.planes.forEach((plane) => {
        ...
        // Hide other planes if one plane is focused
    ...
    store.sensors.forEach((sensor) => {
      ...
      // ✅ ISOLATION: Hide other sensors' tooltips if a specific sensor is focused
  ...
  // ✅ IMMERSIVE NATIVE 3D HEATMAP ENGINE & ZOOM FIX
  useEffect(() => {
    if (
      !isMapReady ||
    ...
    if (isImmersiveActive && focusedSensor) {
      ...
      // ✅ ZOOM FIX: Use explicit 2D Center + Zoom Level to aggressively zoom into the sensor
      const focusCoords = getOffsetCoords(focusedSensor);
      viewRef.current?.goTo(
        {
          center: [focusCoords.lon, focusCoords.lat],
          heading: 0,
          tilt: 75,
          zoom: 21,
        },
  ...
  useEffect(() => {
    ...
    const toRemove = sensorLayerRef.current.graphics.filter((g) => {
      // ✅ ISOLATION: Use !! to force selectedEntityId into a strict boolean
      const isAnotherSelected =
        selectedEntityType === "sensor" &&
        !!selectedEntityId &&
        selectedEntityId !== g.attributes.id;

      // ✅ Force the entire return statement to be a strict boolean
      return Boolean(
    ...
    sensors.forEach((sensor) => {
      if (mapFilters[sensor.type] === false) return;

      // ✅ ISOLATION: Skip rendering this cylinder if a different sensor is selected
      const isAnotherSelected =
        selectedEntityType === "sensor" &&
        selectedEntityId &&
        selectedEntityId !== sensor.id;
      if (isAnotherSelected) return;
